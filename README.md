...
clona el repositorio [tu-usuario]/claude-commands usando gh CLI,
...
> **Nota:** Sustituye `[tu-usuario]` por tu nombre de usuario de GitHub antes de ejecutarlo.
...
```bash
gh repo clone [tu-usuario]/claude-commands /tmp/mis-claude-commands \
  && mkdir -p ~/.claude/commands \
  && cp /tmp/mis-claude-commands/*.md ~/.claude/commands/ \
  && rm -rf /tmp/mis-claude-commands \
  && echo "Comandos instalados correctamente en ~/.claude/commands/"
```

> **Nota:** Sustituye `[tu-usuario]` por tu nombre de usuario de GitHub.  
